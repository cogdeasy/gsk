...
**Overall result: PASS** (0 failed of 37 checks)
...
`Warnings` counts only the records that load. A warning on a held record is in the exception pack and comes back when the reject is settled, but there is nothing to do about it while the record is out of the wave.
...
| REC-PRD-KEY | every product loads once, under an unpadded number | 22 product rows | 22 distinct bare numbers | asserted | PASS |  |
